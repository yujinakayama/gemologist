...
RSpec.shared_context 'with gemspec', :gemspec do
  ...
  let(:gemspec_path) { 'gemologist.gemspec' }

  let(:gemspec_source) { <<-END.strip_heredoc }
...
RSpec.shared_context 'with Gemfile', :gemfile do
  let!(:gemfile) do
  ...
  let(:gemfile_source) { <<-END.strip_heredoc }
...
RSpec.shared_context 'with Gemfile.lock', :lockfile do
  let!(:lockfile) do
  ...
  let(:lockfile_source) { <<-END.strip_heredoc }
    PATH
      remote: .
      specs:
        gemologist (0.0.1)
